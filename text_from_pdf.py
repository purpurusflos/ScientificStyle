...
# Извлекаем текст из указанного текстового элемента.
        ...
        if isinstance(text_line, LTTextContainer):
            for character in text_line:
                if isinstance(character, LTChar):
                    # Добавляем в список название и размер шрифта для каждого символа в строке
...
# Очищаем текст от знаков переноса на новую строку и его объединяем в одну строку.
def clean_text(page_content):
    ...
    text2 += clean_text2
    # Возвращает строку с текстом, готовому к дальнейшему анализу.
    return text2


# Получаем текст основной части статьи без списка литературы.
    ...
    for page_number, page in enumerate(extract_pages(file_path)):
        ...
        for i, element in enumerate(page_elements):
            ...
            if isinstance(element, LTTextContainer):
                ...
                # Проверяем, относится ли данный текст к основной части, а не заголовкам и прочим надписям.
                ...
                # Останавливаем извлечение текста, если в строке есть фраза "Список литературы".
                if "Список литературы" in line_text:
                    return clean_text(page_content)
    # Возвращает строку с текстом основной части статьи.
    return clean_text(page_content)
